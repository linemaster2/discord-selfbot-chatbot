...
class Chat {
        ...
        if (request.status() === 200) {
            try {
                ...
                if (replies && Array.isArray(replies)) {
                    ...
                    console.error("Error: Response replies is undefined or not an array.");
                    return null;
                }
            ...
            throw Error("Failed sending message.");
        }
    }
    // image interaction
        ...
        try {
            if (!mimeType) {
                if (content.startsWith('http') || content.startsWith('https')) {
                    ...
                    return await this.uploadBuffer(Buffer.from(imageResponse.data, "binary"), mimeType);
                } else {
                    const image = await jimp.read(content);
                    return await this.uploadBuffer(image.getBase64(mimeType || "image/png"), mimeType);
                }
            }
        } catch (error) {
    ...
    async uploadBuffer(buffer) {
        try {
            ...
            if (request.status() === 200) {
                ...
                throw new Error("Failed uploading image. Status: " + request.status());
            }
        } catch (error) {
            throw new Error(`Error uploading image buffer: ${error.message}`);
        }
    }    
    // todo
    ...
    async changeToConversationId(conversationExternalId, force = false) {
        ...
        else Error("Could not switch to conversation, it either doesnt exist or is invalid.")
    }
